...
import com.cardinalstar.cubicchunks.mixin.early.common.MixinEntityTracker;
...
public enum Mixins implements IMixins {
    ...
    MIXIN_ANVIL_SAVE_HANDLER(new MixinBuilder("Changing the save handler to return a Cubic anvil chunk loader.")
        ...
        .setApplyIf(() -> true)),
    MIXIN_GAME_REGISTRY_ACCESSOR(new MixinBuilder("Allows access to the generators in GameRegistry")
        .addCommonMixins("common.IGameRegistry")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),

    // CHUNK
    MIXIN_CHUNK_COLUMN(new MixinBuilder("Mixin for making chunks into columns.")
        .addCommonMixins("common.MixinChunk_Column")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    ...
    MIXIN_WORLD(new MixinBuilder("Implementing ICubicWorld.")
        ...
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    MIXIN_WORLD_INFO(new MixinBuilder("Implementing ICubicWorldInfo")
        .addCommonMixins("common.MixinWorldInfo")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    ...
    MIXIN_BLOCK_ACCESS_MIN_MAX_FIX(new MixinBuilder("Have IBlockAccess implement IMinMaxHeight")
        ...
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    MIXIN_WORLD_HEIGHT_LIMIT(new MixinBuilder("Fix a ton of height limit issues in World.")
        .addCommonMixins("common.MixinWorld_HeightLimit")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    MIXIN_WORLD_TICK(new MixinBuilder("Redirecting some things to use Y values.")
        .addCommonMixins("common.MixinWorld_Tick")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    // ENTITY
    MIXIN_ENTITY_DEATH_FIX(new MixinBuilder("Replace -64 constant, to avoid killing entities below y=-64")
        ...
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    MIXIN_ENTITY_TRACKER(new MixinBuilder("Changing the EntityTracker to work with cubes.")
        .addCommonMixins("common.MixinEntityTracker")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    MIXIN_ENTITY_TRACKER_ENTRY(new MixinBuilder("Changing entityTrackerEntry to use cubes.")
        .addCommonMixins("common.MixinEntityTrackerEntry")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    MIXIN_SPAWNER_ANIMALS(new MixinBuilder("Fixing spawner animals to work with cubes.")
        .addCommonMixins("common.MixinSpawnerAnimals")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    MIXIN_STRUCTURE_START(new MixinBuilder("Giving Y position to structures.")
        .addCommonMixins("common.MixinStructureStart")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    // SERVER
    ...
    MIXIN_SERVER_CONFIGURATION_MANAGER(new MixinBuilder("Implements ICubicPlayerList in ServerConfigurationManager")
        ...
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    MIXIN_REGION_FILE_CACHE(new MixinBuilder("I believe this is for compat but IDK")
        .addCommonMixins("common.MixinRegionFileCache")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
    ...
    MIXIN_DEDICATED_PLAYER_LIST(new MixinBuilder("Allow to set vertical view distance.")
        .addServerMixins("server.MixinDedicatedPlayerList")
        .setPhase(Phase.EARLY)
        .setApplyIf(() -> true)),
